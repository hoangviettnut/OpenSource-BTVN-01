...
<img width="1483" height="762" alt="image" src="https://github.com/user-attachments/assets/31fa209b-df1d-4471-a90c-c0ec8cbab5e7" />
...
docker rmi <image>          # Xóa image
 ...
 --- CONTAINERS ---
docker run -d -p 8080:80 <image>  # Tạo & chạy ngầm, map port (VD: nginx)
docker ps                   # Xem các container ĐANG CHẠY
docker ps -a                # Xem TẤT CẢ container (cả đã tắt)
docker stop <container>     # Dừng container
docker start <container>    # Bật lại container đã dừng
docker rm <container>       # Xóa container (phải stop trước)
docker rm -f <container>    # Ép xóa ngay lập tức

 --- DEBUG ---
docker logs <container>              # Xem log của container
docker exec -it <container> bash     # Chui vào bên trong container gõ lệnh (hoặc dùng 'sh')7
### Với Docker Compose
